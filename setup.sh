#!/bin/bash

echo "🚀 Setting up MERN Stack Kanban Board..."

# Create backend .env file
echo "📝 Creating backend environment file..."
cat > backend/.env << EOF
# Server Configuration
NODE_ENV=development
PORT=5000
HOST=localhost

# Database Configuration
MONGODB_URI=mongodb://localhost:27017/kanban-board
MONGODB_TEST_URI=mongodb://localhost:27017/kanban-board-test

# JWT Configuration
JWT_SECRET=your-super-secret-jwt-key-change-this-in-production-12345
JWT_EXPIRE=7d
JWT_REFRESH_SECRET=your-super-secret-refresh-key-change-this-in-production-67890
JWT_REFRESH_EXPIRE=30d

# CORS Configuration
CORS_ORIGIN=http://localhost:3000
CORS_CREDENTIALS=true

# Rate Limiting
RATE_LIMIT_WINDOW_MS=900000
RATE_LIMIT_MAX_REQUESTS=100

# Security
BCRYPT_ROUNDS=12
SESSION_SECRET=your-session-secret-key-change-this-in-production-abcdef

# File Upload (if needed later)
MAX_FILE_SIZE=5242880
UPLOAD_PATH=./uploads

# Email Configuration (for future features)
EMAIL_SERVICE=gmail
EMAIL_USER=your-email@gmail.com
EMAIL_PASS=your-app-password

# Frontend URL
FRONTEND_URL=http://localhost:3000

# API Configuration
API_VERSION=v1
API_PREFIX=/api

# Logging
LOG_LEVEL=info
LOG_FILE=./logs/app.log

# Redis (for future caching)
REDIS_URL=redis://localhost:6379

# Environment specific settings
NODE_TLS_REJECT_UNAUTHORIZED=0
EOF

# Create frontend .env file
echo "📝 Creating frontend environment file..."
cat > frontend/.env << EOF
# API Configuration
VITE_API_URL=http://localhost:5000/api

# App Configuration
VITE_APP_NAME=Kanban Board
VITE_APP_VERSION=1.0.0

# Environment
VITE_NODE_ENV=development
EOF

echo "✅ Environment files created successfully!"
echo ""
echo "🔧 Next steps:"
echo "1. Install backend dependencies: cd backend && npm install"
echo "2. Install frontend dependencies: cd frontend && npm install"
echo "3. Start MongoDB: mongod"
echo "4. Start backend: cd backend && npm run dev"
echo "5. Start frontend: cd frontend && npm run dev"
echo ""
echo "🌐 Access the application at:"
echo "   Frontend: http://localhost:3000"
echo "   Backend API: http://localhost:5000/api"
echo ""
echo "🔑 Demo credentials:"
echo "   Email: john@example.com"
echo "   Password: password123"
